...
=======
    ...
    for (var i = 0; i < structure.levels.length; i++) {
      ...
      schedule.add(_ScheduleRow.level(index: i, startMins: elapsed));
    ...
    final plannedLadder = structure.levels
        .take(structure.effectivePlannedLevels)
        .toList();
    final growthPerLevel =
        plannedLadder.length >= 2 && plannedLadder.first.bb > 0
        ? math
              .pow(
                ...
                children: [
                  ...
                  AppButton(
                    ...
                    onPressed: () async {
                      ...
                      context.read<AppProvider>().generateFinalStructure(
                        checkedInCount,
                      );
          ...
          if (_tab == _tabParams) ...[
            ...
            LayoutBuilder(
              builder: (context, constraints) {
                final cardWidth = constraints.maxWidth < 400
                    ? constraints.maxWidth
                    : (constraints.maxWidth / 3).floorToDouble().clamp(
                        100.0,
                        180.0,
                      );
                return Wrap(
                  ...
                  Row(
                    children: [
                      ...
                      AppButton(
                        ...
                        variant: AppButtonVariant.ghost,
                        onPressed: () =>
                            _showAdjustModal(context, app, structure),
                        child: const Text('Adjust'),
                      ),
                    ],
                  ),
                      ...
                      Builder(
                        builder: (context) {
                          ...
                          final isAnteStart =
                              l.ante != null &&
                              (i == 0 || structure.levels[i - 1].ante == null);
                          final bbDepth = (structure.startingStack / l.bb)
                              .round();
                          return Container(
                            padding: const EdgeInsets.symmetric(vertical: 6),
                            decoration: BoxDecoration(
...
void _showAdjustModal(
  ...
  final chipValues =
      (app.currentGame?.settings.chipSet ?? const [])
          .map((c) => c.value)
          .where((v) => v > 0)
          .toList()
        ..sort();
  ...
  final step = rawStep <= minChip ? minChip : (rawStep ~/ minChip) * minChip;
          ...
          Row(
            children: [
              ...
              Expanded(
                child: AppButton(
                  onPressed: () {
                    final summary = app.adjustStructure(startingStack: stack);
...
Widget _buildC2ScheduleRow(_ScheduleRow row, TournamentStructure structure) {
  if (row.isBreak) {
    return Padding(
      ...
      child: Row(
        children: [
          const Icon(Icons.coffee_outlined, size: 18, color: Color(0xFF4CAF50)),
...
class _GenerationParamsCardState extends State<_GenerationParamsCard> {
  ...
  void _apply() {
    ...
    final rebuyChips = _read(
      _rebuyChips,
      _rebuyChipsOf(widget),
      min: 1,
      max: 100000000,
    );
    final reEntryChips = _read(
      _reEntryChips,
      _reEntryChipsOf(widget),
      min: 1,
      max: 100000000,
    );
    final addOnChips = _read(
      _addOnChips,
      _addOnChipsOf(widget),
      min: 1,
      max: 100000000,
    );
      ...
      child: Text(
        ...
        style:
            AppTypography.mono(
              ...
              shadows: [Shadow(color: AppColors.shadowDeep, blurRadius: 2)],
            ),
      ),
    );
  }
                ...
                ? Column(
                    ...
                    children: [
                      ...
                      for (var i = 0; i < rows.length; i++) ...[
                        if (i > 0)
                          Divider(color: AppColors.hairlineBorder, height: 1),
                        Padding(
                          padding: const EdgeInsets.symmetric(
                            horizontal: AppSpacing.lg,
                            vertical: 10,
