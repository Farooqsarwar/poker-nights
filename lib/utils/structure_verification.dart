  ...
  static StructureAudit audit(LiveGame game) {
    ...
    if (stored.engineVersion != TournamentEngine.engineVersion) {
      return StructureAudit.cannotVerify(
        'generated by engine v${stored.engineVersion}; this device runs v${TournamentEngine.engineVersion}',
      );
    }
    ...
    try {
      fresh = TournamentEngine.generate(
        TournamentParams(
          ...
          // republished as three would otherwise produce a ladder nobody could
          // challenge, because the recomputation would never have been told
          // how the field was split. Same argument as `format` itself.
          shootoutTables: s.shootoutTables,
          shootoutTableTargetMins: s.shootoutTableTargetMins,
          // §3. An undeclared bonus stack changes the chips in play, so it
          // changes the ladder — it has to be declared or it reads as tampering.
